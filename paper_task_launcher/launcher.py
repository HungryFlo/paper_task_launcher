...
任务：创建一个对用户友好的前端网页，辅助用户阅读、理解和学习这篇论文。在保留论文关键信息的前提下，以渐进方式帮助用户学习。
...
def launch_task(
    ...
    *,
    ...
    if shutil.which(codex_bin) is None and not prepare_only:
        raise LauncherError(f"Codex executable not found: {codex_bin}")
    task = prepare_task(workspace_value, paper_value, progress=progress)
    ...
    task.manifest["codex_started_at"] = utc_now()
    atomic_json(recording_dir / "manifest.json", task.manifest)
    ...
    task.manifest["state"] = "completed" if return_code == 0 else "codex_failed"
    task.manifest["codex_exit_code"] = return_code
    task.manifest["completed_at"] = utc_now()
    atomic_json(recording_dir / "manifest.json", task.manifest)
    if progress:
        progress(
            f"Codex 会话已结束，共记录 {task.manifest.get('turn_count', 0)} 轮；"
